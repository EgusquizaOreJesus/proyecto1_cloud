...
export const Overview = ({userId}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${backendUrl}/estados/find_user/` + userId);
  ...
  const renderContent = (user) => {
    return user.contenido.map(item => {
      console.log(item);
  ...
  const handlePublishClick = () => {
    ...
    try {
      const response =  axios.post(`${backendUrl}/estados/save/`+userId, { contenido: textContentArray });
      if (response.status === 200) {
                      ...
                      <div className='header'>
                        <a href="/" className='user'>
                              <div className='avatar' style={{
                                    backgroundImage: `url(${item.enlace_imagen !== null ? item.enlace_imagen : '../../../images/profile/profile.png'})`,
                                  }}    
                              />{item.nickname}
                          </a>
                          <div className='info'>
